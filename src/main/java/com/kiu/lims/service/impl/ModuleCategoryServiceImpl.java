...
public class ModuleCategoryServiceImpl implements ModuleCategoryService {
    ...
    @Override
    public ResponseModel updateModuleCategory(ModuleCategoryEntity moduleCategory) {
        ResponseModel responseModel = new ResponseModel();

        try {
            Long categoryId = moduleCategory.getCategoryId();
            if (categoryId != null) {
                Optional<ModuleCategoryEntity> optionalCategory = moduleCategoryRepository.findById(categoryId);
                if (optionalCategory.isPresent()) {
                    ModuleCategoryEntity updatedCategory = moduleCategoryRepository.save(moduleCategory);
                    responseModel.setCode(200); // OK
